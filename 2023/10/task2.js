...
let startChar = {
    above: above ? tiles[above[0]][above[1]] : false,
    below: below ? tiles[below[0]][below[1]] : false,
    left: left ? tiles[left[0]][left[1]] : false,
    right: right ? tiles[right[0]][right[1]] : false
}

let newStartChar = undefined
if (startChar.above && startChar.below) {
    newStartChar = '|'
} else if (startChar.left && startChar.right) {
    newStartChar = '-'
} else if (startChar.above && startChar.right) {
    newStartChar = 'L'
} else if (startChar.above && startChar.left) {
    newStartChar = 'J'
} else if (startChar.below && startChar.left) {
    newStartChar = '7'
} else if (startChar.below && startChar.right) {
    newStartChar = 'F'
}

let reworked = (scores.map((row, rowindex) => {
